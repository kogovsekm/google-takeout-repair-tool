...
- macOS Apple Silicon: use `Google Takeout Repair Tool-1.0.6-arm64.dmg`
- macOS Intel: use `Google Takeout Repair Tool-1.0.6.dmg`
- Windows 64-bit: use `Google Takeout Repair Tool Setup 1.0.6.exe`
- Linux 64-bit: use `Google Takeout Repair Tool-1.0.6.AppImage`
...
- Linux: the AppImage may not start until it has executable permission. If needed, run `chmod +x "Google Takeout Repair Tool-1.0.6.AppImage"` and launch it again.
...
Folder options (mutually exclusive — pick one per run):

- Flatten months into years: moves files from YYYY/MM sub-folders up into their parent YYYY folder.
- Flatten all data to selected folder: recursively moves every file from any sub-folder directly into the selected root folder, regardless of folder names or nesting depth.
- Flatten data into halves: recursively scans all files and distributes them across two sub-folders (H1 and H2) balanced by total size using a greedy largest-first algorithm. Every file is always included.

Supporting options (compatible with any folder option):

- Remove empty folders: sweeps and removes any empty directories left after the chosen folder operation.
- Create temporary folder for review: runs the operation inside a temporary sub-folder first so you can inspect the result before applying it. Includes an explicit apply step with rollback on failure.

Additional behaviours:

- No folder option is pre-selected; Start Organising is disabled and a warning is shown until one is chosen.
- Mutual exclusivity is enforced in the UI — selecting one folder option automatically deselects the others.
- All options are locked during an active run.
- In-place moves with collision-safe destination names.
- Separate progress channel and dedicated organise report dialog showing moved file counts, removed folder counts, and (for halves runs) the H1/H2 file count and size breakdown.
...
npm run release:all -- 1.0.6  # bump version, build macOS+Windows+Linux installers, regenerate SHA256SUMS
